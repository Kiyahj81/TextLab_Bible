...
`npm run smoke:classifier` is a live, pre-merge smoke test for the LLM complexity classifier (`lib/ai/complexity.ts`): it exercises the exact production call (Responses API, strict JSON schema, reasoning effort "low", no retry) against the real model over a small fixed prompt set. It gates on the classifier **contract** and **always validates verdicts**: each prompt is tried first at the mandated 2 s production budget, and if that times out the verdict is still validated via a relaxed-timeout probe. It exits non-zero on a wrong verdict, a non-timeout API error (400 / bad JSON / missing key), or no verdict even under the relaxed probe. Calls that only completed over the 2 s budget are reported as a latency datapoint to re-check in a production-like network (in production a classifier timeout falls back to the deterministic score, and dev-box round-trip latency is not signal about the classifier itself). It requires `OPENAI_API_KEY`.
