...
## Forward-looking: gpt-5.4-mini as the router model (evaluated 2026-07-02, not adopted)
...
- **Caveat:** with gpt-5.4-mini, `routing-why-alone-default` routed **default** (agreeing with the score), *not* scholarly — so the schema/golden-set note that "the LLM consistently routes this scholarly" is **gpt-5-mini-specific** and would need revisiting if the model is switched.

Verdict: gpt-5.4-mini is a strong, near-free improvement (lower/tighter latency → fewer timeout fallbacks → the router's intended behavior actually happens), captured by setting `OPENAI_ROUTER_MODEL=gpt-5.4-mini` with no source change. **Not adopted in this branch** — left as a follow-up decision; adopting it should re-touch the why-alone routing note.
