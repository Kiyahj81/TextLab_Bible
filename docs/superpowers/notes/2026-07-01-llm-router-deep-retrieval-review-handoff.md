...
- **API migration:** `confirmEscalation` = ask-first opt-out; auto-escalation is the default when unset. `autoEscalate` is not an option on `answerBibleQuestion`. (Post-review, Fable #1: the deprecated `autoEscalate` request param + its `false → confirmEscalation:true` mapping were removed — it guarded a request shape no shipped client sent; a legacy key is now stripped by zod and falls through to the auto default, with the durable opt-out carried by the cookie migration.)
...
- **`autoEscalate` was removed from the API post-review** (Fable finding #1): the shipped UI only ever sent `autoEscalate: true` or omitted the field, so the `false → confirmEscalation:true` mapping guarded a phantom request shape. A stale client's key is now stripped and falls through to the auto default; the durable legacy opt-out is carried by the confirm-scholarly cookie migration.
