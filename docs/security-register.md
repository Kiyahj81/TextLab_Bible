...
| Reason | The retrieval-scoping change returns whole passages in full, so a deterministic-fallback answer overflowed the old caps and 400'd on save. A single chapter is ~13 KB; the measured worst case at the time was a multi-reference prompt at the planner's standard 8-passage-call ceiling (4 large chapters × 2 corpora), measured on the full corpus at ~57 KB answer / ~58 KB markdown / ~167 KB request body. Caps sized to that ceiling with margin. |
| Status | Accepted — bounded (standard); deep-mode size unvalidated — hardening debt |
| Mitigation | Endpoint is behind auth (`requireAuth`) and same-origin (`assertSameOrigin`); `readJsonLimited` still rejects bodies over the 256 KB cap with 413. The upstream answer/markdown sizes are hard-bounded by the planner's deterministic call cap × `MAX_PASSAGE_LINES`, so payloads cannot grow unboundedly for a given cap — a 6-reference prompt produces the same size as a 4-reference one at the same cap. |
| **2026-07-01 update — LLM router + deep retrieval** | The deterministic call cap that bounds this ceiling was renamed and split: `MAX_PLANNED_CALLS` → `MAX_PLANNED_CALLS_STANDARD` (8, unchanged value) plus a new `MAX_PLANNED_CALLS_DEEP` (12), both in `lib/ai/retrievalPlanner.ts`. **Deep retrieval** (live scholarly routing — auto-escalated by the LLM/weighted-score complexity router, or user-confirmed escalation) now raises the deterministic ceiling from 8 to **12** passage calls, **plus an additional corpus-wide semantic pass** that standard retrieval does not run unconditionally. The **non-live deterministic-fallback path stays at the standard 8-call cap**: routing never runs when live synthesis is off (`isLiveAssistantEnabled()` false), so `deep` is always `false` and the 8-call ceiling this row's caps were originally sized against still holds there. The risk is narrower than "any answer can now be bigger": it is specifically the **live-scholarly path's deterministic-fallback answer** (`buildDeterministicFallback`, reached when live synthesis throws or returns nothing after routing already selected deep retrieval) that can embed the full 12-call-plus-semantic-pass evidence and therefore exceed the 8-call worst case this entry's `MAX_ANSWER`/`MAX_MARKDOWN`/`NOTE_BODY_LIMIT` figures were measured against. |
| Outstanding hardening debt | The deep-mode worst-case fallback size (12 calls + corpus-wide semantic pass, on the live-scholarly path) has **not** been re-measured. `MAX_EVIDENCE_CHARS = 58_000` still bounds the assembled evidence text embedded in the fallback answer, so growth is not unbounded, but whether `MAX_ANSWER` (64 KB) / `MAX_MARKDOWN` (72 KB) / `NOTE_BODY_LIMIT` (256 KB) still hold with adequate margin at the new deep ceiling is unvalidated — re-measure (e.g. via `scripts/evidence-diff.ts` or a manual worst-case prompt) and resize the caps if needed before treating deep mode as fully hardened. **Exposure note (2026-07-02, CodeRabbit PR #55):** auto-escalation is now the *default* (`confirmEscalation` opt-out), so the live-scholarly deep path is reachable without an explicit opt-in — the debt is more likely to surface in production, which raises the priority of the re-measure below (still bounded by `MAX_EVIDENCE_CHARS`, so not a merge blocker). |
| Owner | Maintainer (kiyahj81) |
| Opened | 2026-05-30 (retrieval-scoping branch); deep-mode ceiling noted 2026-07-01 (LLM router + deep retrieval) |
| Next review | Re-measure the live-scholarly deterministic-fallback worst-case size and confirm/resize `MAX_ANSWER` / `MAX_MARKDOWN` / `NOTE_BODY_LIMIT` for the 12-call + corpus-wide-semantic-pass deep ceiling. |
...
| Reason | A full grounded answer (structured JSON with Greek) routinely takes longer than 25 s to generate, so the client abandoned responses the model had already completed server-side and surfaced "Request timed out" → the local fallback. **2026-07-02 update:** the output budget is now role-aware — default `OPENAI_MAX_OUTPUT_TOKENS`=2400, **scholarly=6000** (`OPENAI_SCHOLARLY_MAX_OUTPUT_TOKENS`). A 6000-token scholarly answer takes ~2.5× longer to generate, so it consumes more of the 120 s client-timeout / worker-hold budget (still bounded by it, and by the rate limiter). If scholarly generations routinely approach 120 s, prioritize the streaming resolution below rather than raising the timeout further. |
