...
### Notes & Assistant accessibility parity (2026-06)

The `/notes` and `/assistant` pages were brought in line with the aria-live + accessible-name conventions established on `/read` and `/search`. In `NotesPanel`, the per-note title input and body textarea gained `aria-label`s (they previously had only a placeholder / nothing), the per-note save/delete status and the result-count summary became `role="status" aria-live="polite"` regions, and the decorative Save/Trash icons are `aria-hidden`. In `AiAssistant`, the "Question" label is now associated with the prompt textarea (`htmlFor`/`id`), the read-only Markdown output got an `aria-label`, the save-status message became an always-rendered `aria-live` region (matching `/search`), submit errors use `role="alert"`, and visually-hidden `aria-live` regions announce the "Copied" result of the retrieval-trace / citations copy buttons (whose static `aria-label` otherwise suppressed the state change). `tests/unit/components/a11y-notes-assistant.test.tsx` pins the accessible names and live regions.

### LLM complexity router + deep retrieval (2026-07-01, branch `feat/assistant-llm-router-deep-retrieval`)

Reorders the assistant pipeline so **routing runs before retrieval**, letting a scholarly routing decision widen retrieval instead of retrieval always running at one fixed depth. `lib/ai/complexity.ts` adds a live LLM classifier `classifyComplexityLLM` (OpenAI Responses API, model from `OPENAI_ROUTER_MODEL` — default `gpt-5.4-mini` — strict JSON schema output, reasoning effort "low", 3s timeout, no retry) alongside the existing deterministic weighted `scoreComplexity` (threshold 2, unchanged cue set). `routeAssistantPrompt` (`lib/ai/modelRouter.ts`) is now **async** and returns a structured `RoutingDecision` (`modelRole`, `modelUsed`, `routingDecision`, `deep`, `routerSource`, `complexityScore?`): in live mode it calls the classifier first and falls back to the score on any failure (timeout, API error, malformed JSON) — `routerSource` is one of `"llm" | "score" | "score-fallback" | "manual" | "none"`, always naming who decided. Complex questions **auto-escalate to the scholarly model by default**; the assistant UI toggle flipped from opt-in "auto-escalate" to **ask-first** ("Ask before using the scholarly model when a question looks complex", sends `confirmEscalation`), with a one-time migration of the legacy auto-scholarly cookie. Production non-live requests still never route (`modelUsed: "none"`, `routerSource: "none"`, `deep: false`) — routing only runs when `OPENAI_API_KEY` is set and `TEXTLAB_ASSISTANT_DISABLE_LIVE` is unset.

Scholarly routing (`deep: true`) widens `runRetrievalPlan`: the deterministic call cap raises from `MAX_PLANNED_CALLS_STANDARD = 8` to `MAX_PLANNED_CALLS_DEEP = 12`, and the planner always adds a corpus-wide `semanticCall` (in addition to a scoped one when the prompt pins a book/chapter) regardless of the standard `shouldRunSemantic` gate, since a deep interpretive question can be pinned to exact verses or have no extractable topic words yet still need cross-text evidence. New `npm run smoke:classifier` (`scripts/smoke-classifier.ts`) is a live, pre-merge merge-blocker smoke test that exercises the exact production classifier call against the real model over a small fixed prompt set. The eval gate (`scripts/eval-gate.ts`) now asserts each golden-set item's `expectedRouting` where present, and the eval runner routes before retrieving so `deep` is honored end-to-end. Unit coverage: `tests/unit/lib/ai/complexity.test.ts`, `tests/unit/lib/ai/modelRouter.test.ts` (extended), plus `retrievalPlanner`/`assistant`/eval-dataset test updates for the deep call cap and routing-first ordering.

This raises the deterministic-fallback answer's worst-case evidence size beyond the previously measured 8-call ceiling on the live-scholarly path (the non-live path is unaffected — routing never runs there, so `deep` never applies). See the "Generated-study-notes request body limit raised to 256 KB" entry in `docs/security-register.md`, updated to record the new deep ceiling and the outstanding hardening debt that this larger worst case has not yet been re-measured against the `generated-study-notes` caps.
...
| `/signin` | Custom sign-in page; styled card matching the editorial-scholar palette. Renders the dev-credentials form (when `AUTH_DEV_ENABLED=1`) and a GitHub OAuth button (when `GITHUB_CLIENT_ID/SECRET` are set). GitHub sign-in is gated by a fail-closed allowlist (`GITHUB_ALLOWLIST`, numeric account ids) — off-list accounts get an "invite-only" `AccessDenied` message. Redirects authenticated visitors back to `/read`. Surfaces inline errors from server-action validation and Auth.js `?error=` redirects. |
...
The assistant route follows a route-then-retrieve contract — routing runs first so a scholarly decision can widen retrieval:

1. **Route the prompt** (`routeAssistantPrompt` in `lib/ai/modelRouter.ts`) — returns a structured `RoutingDecision` (`{ modelRole, modelUsed, routingDecision, deep, routerSource, complexityScore? }`). Manual escalation (`escalate: true`, user-requested) is always honored (`routerSource: "manual"`, `deep: true`). Otherwise, in live mode the router calls the LLM complexity classifier `classifyComplexityLLM` (`lib/ai/complexity.ts`, OpenAI Responses API, model from `OPENAI_ROUTER_MODEL` — default `gpt-5.4-mini` — strict JSON schema output, reasoning effort "low", 3s timeout, no retry) and falls back to the deterministic weighted `scoreComplexity` (threshold 2) on any classifier failure (`routerSource: "llm"` vs `"score-fallback"`); outside live mode (or when live synthesis is off entirely) it uses the score directly (`routerSource: "score"` / `"none"`). A question judged complex **auto-escalates to the scholarly model by default** (`deep: true`); the assistant UI's "ask before using the scholarly model when a question looks complex" preference (`confirmEscalation`) instead returns a `recommendedUpgrade` advisory and stays on the default model until the user confirms. A legacy auto-scholarly cookie migrates one-time into the new ask-first preference. Production requests with live synthesis disabled never route at all — `routerSource: "none"`, `modelUsed: "none"`, `deep: false` — and always answer with the deterministic fallback at standard retrieval depth.
2. **Run retrieval** (`extractSignals` → `runRetrievalPlan(signals, prompt, { semanticEnabled, deep: routing.deep })`) — the Paradigm C deterministic-first pipeline (`lib/ai/signals.ts`, `lib/ai/retrievalPlanner.ts`): signals are extracted from the prompt, `buildPlan` turns them into a bounded set of DB-backed calls (passage/lemma/keyword/morphology/top-lemmas), and `runRetrievalPlan` executes them into an `EvidencePacket` (citations, tool trace, formatted evidence). Signal extraction parses cross-chapter references (e.g. `2 Cor 4:16–5:10`) via `ParsedReference.chapterEnd`, matches book aliases longest-first, strips parsed references/book aliases/morphology codes before topic extraction, preserves known and quoted multi-word phrases as `phraseTerms`, and maps stable entity topics (`jesus`, `christ`, `god`, `lord`, `jerusalem`) to Greek lemmas instead of dropping them as proper nouns. The retrieval planner applies these completeness rules with bounded, safe assembly: (a) word searches (`searchLemma`/`searchKeyword`/`searchMorphology`) are scoped to a chapter only for exactly one single-chapter reference; multiple references in one book use book scope, and cross-book prompts stay corpus-wide; (b) large surveys return 25 results plus the true total count (e.g. `searchLemma(ἀγάπη) — 116 hit(s) … (91 more)`), while small ones (≤ 25 hits) return all results; (c) `passageCall` assembles complete passages — whole chapters, in-chapter ranges, and cross-chapter ranges built from per-chapter `getPassage` calls — subject to a `MAX_PASSAGE_LINES = 120` hard ceiling with a `…(N more)` marker. Cross-chapter expansion is bounded against pathological prompts (the chapter span is clamped, the fetch loop stops once a chapter returns no rows, it probes past a leading out-of-range segment so a valid tail is not dropped, the tool trace lists only chapters actually fetched, and an explicit truncation marker is shown when the ceiling cuts a span short). (d) The total assembled evidence is capped at `MAX_EVIDENCE_CHARS = 58_000` (with an honest omission marker): the deterministic-fallback answer embeds the evidence verbatim and the user may save it, so this keeps that answer within the generated-study-note persistence caps by construction — single/few-reference answers are never trimmed; only a pathological multi-range prompt is. (e) **Phase 4a semantic path**: on conceptual prompts with real topic words or phrase terms, the planner also runs a gated `semanticCall` outside the deterministic call budget (`MAX_PLANNED_CALLS_STANDARD = 8`; `MAX_PLANNED_CALLS_DEEP = 12` when `deep` is set by scholarly routing) — `searchSemantic` fuses pgvector KNN over current-model WEB verse embeddings with keyword FTS via Reciprocal Rank Fusion (`lib/search/rrf.ts`), filters hits to the SBLGNT spine, and expands each hit to an on-spine ±2 verse window. Semantic search skips morphology-only prompts and exact-verse lookups, honors the same book/chapter scope as deterministic retrieval, requires `OPENAI_API_KEY`, and gracefully no-ops to deterministic retrieval when unset or disabled. **Deep mode** (2026-07-01, `feat/assistant-llm-router-deep-retrieval`) additionally always adds a corpus-wide `semanticCall` regardless of `shouldRunSemantic` — a pinned-scope prompt still gets a scoped pass plus a corpus-wide one, and an unscoped deep prompt gets a single wider corpus-wide pass (`SEMANTIC_HIT_LIMIT_DEEP_UNSCOPED = 10` vs the standard `SEMANTIC_HIT_LIMIT = 5`) — because a deep interpretive question pinned to exact verses, or with no extractable topic words, still needs cross-text evidence that a scope-limited pass cannot supply. (f) **Phase 4b rerank**: after the SBL-spine filter, `searchSemantic` reranks the candidate pool (top-30 → top-5) on WEB text via Voyage rerank-2.5 through Vercel AI Gateway (`lib/search/rerank.ts`, AI SDK `rerank`), gated on `AI_GATEWAY_API_KEY`; on missing key / error / timeout it falls back to the RRF order (graceful no-op), so behavior is unchanged when the key is unset.
...
- **Unit and integration tests** (vitest 4 + jsdom + RTL). Coverage gate via v8 over `app/api/**` + `lib/**`. Additions from the LLM router + deep retrieval feature (2026-07-01):
  - `tests/unit/lib/ai/complexity.test.ts` — `scoreComplexity` cue weights/threshold and `classifyComplexityLLM` (client-unavailable throw, live request shape, timeout/no-retry override, schema-mismatch throw)
  - `tests/unit/lib/ai/modelRouter.test.ts` (extended) — async `routeAssistantPrompt`: manual escalation, LLM-classifier success/fallback-on-throw, non-live score path, auto-escalate-by-default vs. `confirmEscalation` ask-first, `routerSource` on every branch
  - `tests/unit/lib/ai/retrievalPlanner.test.ts` (extended) — `deep` call-cap (`MAX_PLANNED_CALLS_DEEP = 12`), corpus-wide semantic pass added in deep mode regardless of `shouldRunSemantic`
  - `tests/unit/lib/ai/assistant.test.ts` (extended) — route-before-retrieve ordering, `deep`/`routerSource`/`complexityScore` propagated onto `AssistantAnswer`
  - `tests/unit/eval/dataset.test.ts` — validates the golden set (including the new optional `expectedRouting` field on the `routing · *` items) against `goldenSetSchema` and the canonical-reference invariant
  - `npm run smoke:classifier` (`scripts/smoke-classifier.ts`) is a separate live pre-merge check against the real classifier model, not part of the vitest suite (see the LLM router + deep retrieval writeup above).

  Additions from Phase 5b (Louw-Nida domain search):
