...
export function NoteList({ notes, onChanged }: { notes: Note[]; onChanged: () => void }) {
  ...
  async function remove(id: string) {
    ...
    setBusy(true);
    try {
      const res = await fetch(`/api/notes/${id}`, { method: "DELETE", headers: { "Content-Type": "application/json" } });
      if (res.ok) onChanged();
      else setStatus("Could not delete note.");
    } catch {
  ...
  async function save(note: Note) {
    ...
    try {
      ...
      const res = await fetch(`/api/notes/${note.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: note.title ?? undefined, body, tags: note.tags }) });
      if (res.ok) { setEditingId(null); onChanged(); }
      else setStatus("Could not save note.");
    } catch {
      setStatus("Network error. Try again.");
    } finally {
      setBusy(false);
  ...
  return (
    <>
      ...
      {status ? (
        <p role="status" aria-live="polite" className="mt-2 font-sans text-sm text-slate-600">{status}</p>
      ) : null}
    </>
  );
}
