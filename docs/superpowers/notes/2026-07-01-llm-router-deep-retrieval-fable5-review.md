...
- **Consequence:** `ROUTER_TIMEOUT_MS` widened 2000 → 3000 ms (`14ea1a1`), revising the round-1 §8.1 "keep 2000 ms" recommendation on new evidence. Spec/README/PROJECT_STATE synced.
- **Second clean run** (GitHub Actions 28602760101, current HEAD's 3000 ms code — auto-triggered once the smoke workflow watched classifier paths, per the Codex finding below): 4/4 correct, but **3 of 4 still exceeded 3000 ms** (one at 4.6 s). gpt-5-mini's latency is wider than the first run implied — ~2.3–2.6 s median with frequent spikes past 3 s. So **3000 ms captures the median, not the tail**: a minority of complex prompts still fall back to the score (graceful, acceptable). Chasing the tail with an ever-larger serial budget has diminishing returns; the real fix is the queued concurrent-retrieval reorder (run retrieval alongside routing, top up on a scholarly verdict), which makes the budget mostly free in wall-clock terms. The complexity.ts comment and spec were corrected to state this honestly rather than "3000 ms covers the distribution."
- **Transient false-red + hardening** (Codex round-3 finding 1): the smoke run at `ea5fca5` (28603016648) went red — but the cause was a `Premature close` connection storm (3 calls dropped, 1 timed out even on the relaxed probe), the same Actions↔OpenAI transient that degraded the eval reports, **not** a code defect (identical code passed at `f8f07c8` and locally). The smoke had been treating a connection drop the same as a real contract error (400/bad-JSON), so a network blip false-reddened a merge-blocker. Fixed: `scripts/smoke-classifier.ts` now retries an isolated transient connection drop once via the relaxed probe and only hard-fails if it **recurs** (persistent fault) — a wrong verdict or a genuine 400/bad-JSON still hard-fails immediately. A full connectivity outage (every call failing, as in that run) still fails, correctly. Re-run confirmed green.
...
## gpt-5.4-mini as the router model (evaluated and ADOPTED 2026-07-02)

Investigating the latency tail (above) led to evaluating `gpt-5.4-mini` (OpenAI's low-latency mini, ~2× faster per its model card, and it exposes a `reasoning.effort: "none"` tier the pinned SDK can't yet use). Router model is already env-selectable (`OPENAI_ROUTER_MODEL`), so this is a zero-code swap. Two clean-runner validations (via a new `router_model` `workflow_dispatch` input on the smoke + weekly-eval workflows):

- **Latency A/B (smoke, run 28605440813, at `effort: "low"`):** gpt-5.4-mini 1383/1474/1673/2205 ms — **0/4 over the 3000 ms budget**, 3 of 4 even under the old 2000 ms — vs gpt-5-mini's typical 1–3 of 4 over 3000 ms (spikes to 4.6 s). Verdicts 4/4 correct.
- **Golden-routing check (weekly-eval, run 28605442980, healthy — 0 `Premature close`):** **all 26 items got a real `llm` verdict, zero score-fallbacks** (gpt-5-mini runs had several fallbacks from classifier timeouts), and **all 6 golden routing items matched their `expectedRouting`**. Routing was identical to the gpt-5-mini after-run except one borderline conceptual item (`sexual-immorality-conceptual`: scholarly here vs default there — an LLM judgment call, not a gated item).
- **Caveat (now handled):** with gpt-5.4-mini, `routing-why-alone-default` routed **default** (agreeing with the score), *not* scholarly — so the earlier schema/golden-set note that "the LLM consistently routes this scholarly" was **gpt-5-mini-specific**. On adoption those notes were rewritten to state the model-dependence (gpt-5-mini scholarly vs gpt-5.4-mini default) rather than a single model's behavior.

Verdict: gpt-5.4-mini is a strong, near-free improvement (lower/tighter latency → fewer timeout fallbacks → the router's intended LLM verdicts actually happen instead of degrading to the score). **Adopted as the router default** — `ROUTER_MODEL_DEFAULT` changed `gpt-5-mini → gpt-5.4-mini` in `lib/ai/complexity.ts`, with the unit-test assertions, the `expectedRouting` routing notes, and all docs (README, PROJECT_STATE, design spec, `.env.example`, workflow input descriptions) updated to match. Still `OPENAI_ROUTER_MODEL`-overridable. The concurrent-retrieval reorder remains queued as the structural latency fix.
