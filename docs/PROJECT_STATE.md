...
- **338 unit tests across 40 files** (vitest 4 + jsdom + RTL). Coverage gate via v8 over `app/api/**` + `lib/**`: lines 87.64%, statements 84.44%, functions 84.15%, branches 73.21%. Major additions from Phase 2 (grounding + Silence Protocol):
  ...
  Additions from retrieval scoping and completeness (`fix/retrieval-scoping`):
