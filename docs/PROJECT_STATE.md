...
| `/search` | Four modes on a **segmented control** (Keyword / Lemma / Morphology / Domain) with per-mode placeholders + hint text, empty-state example chips, and per-mode no-results guidance. **Match highlighting** on result verses; result references **link into the reader** (off-spine keyword hits stay plain text). Mode+scope results label (`116 results for lemma "λόγος" in John`); **morphology tooltips** decode result codes; **friendly pagination** with absolute ranges + page-size select; numeric chapter filter **disabled until a book is chosen**; saved searches show friendly book names and persist the executed mode; aria-live status/rename/delete announcements. Keyword mode uses FTS (`bible_simple` unaccent config, `websearch_to_tsquery`): accent-insensitive Greek, whole-lexeme (not substring), multi-word AND, optional rank ordering. Morphology mode normalizes Robinson-ordered query codes to the stored MorphGNT scheme before matching. **Domain mode** (Phase 5b) queries Louw-Nida semantic domains via a domain dropdown (`33 — Communication`), a dependent subdomain dropdown, and a free-text LN-reference field (`33.55`); saving domain searches is not yet supported. |
...
- **CI + nightly wiring.** Both GitHub Actions workflows are added in `.github/workflows/` and are now
  manually activated: `eval-gate.yml` (deterministic, DB-only, blocking — runs on `pull_request` + `push`
  to `main`) and `nightly-eval-report.yml` (full hybrid pipeline + LLM-judge faithfulness — nightly cron
  + `workflow_dispatch`, uploads `eval/output/` as an artifact). As of 2026-06-09, repo secrets are set
